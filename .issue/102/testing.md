...
- 警告文の検索キーは `did not close within` を想定しているが、**正確な文言は実装時に確定する**ため、実際には `[peek]` プレフィックス付きの警告行が出ていないかを目視でも確認する。
...
- **確認ポイント:**
  ...
  - clack の枠（`┌ Shutting down...` 〜 `└ Server stopped. Bye!`）が崩れていないこと。
...
- **目的:** 1 回目のシャットダウンが有界化されたことで、2 回目の force exit（`process.exit(1)`）が「押しても意味がない保険」として残っているだけで、押した場合に壊れないことを確認する。
- **手順:**
  1. SSE 接続を張った状態で `Ctrl+C` を素早く 2 回連続で押す。
  2. `echo $?` を実行する。
- **期待結果:** `Force exiting...` が表示され、プロセスが即座に終了する。exit code は **1**（1 回だけ押した場合の 0 と異なるのは意図した挙動）。ハングしない。
