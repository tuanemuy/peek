...
- **手順:**
  ...
  4. Network タブを **4 分間**観察する（放置するだけでよい。理由は下記「観察時間を 4 分にする理由」）。
- **期待結果:**
  - Ctrl+C から**ほぼ即座に**（30 秒待たずに）元の `/sse` が完了扱いになり、クライアントの再接続が始まる。
  - 再接続の `/sse` は `ERR_CONNECTION_REFUSED` で失敗する（サーバーはリスナーを既に閉じている）。
  - 再接続の試行間隔が指数的に伸びて 30 秒で頭打ちになり、**10 回目（Ctrl+C から約 181 秒後）を最後に停止して無限には続かない**。
- **観察時間を 4 分にする理由:** 2 つのクライアント実装はどちらも `delay = min(1000 * 2^(n-1), 30000)`、上限 10 回（`src/core/sse-constants.ts`）。試行のタイムラインは Ctrl+C からの経過秒で **1 → 3 → 7 → 15 → 31 → 61 → 91 → 121 → 151 → 181** で、停止するのは 10 回目が失敗した**約 181 秒（≈3 分）後**である。**2 分で観察を打ち切ると 8 回目までしか見えず、「まだ再接続し続けている」正常な状態を FAIL と誤記録しうる。** 「10 回で止まる」ことは自動テストでは確認できずこの項目でしか観測できないのに対し、延長のコストは待機 +2 分（Phase 4 全体で見れば数 % 増）に過ぎないため、期待結果を弱めるのではなく観察時間を 181 秒 + マージンの 4 分に延ばす。
- **確認ポイント:**
  - **「再接続が発生しないこと」を期待しない。** 2 つのクライアント実装はいずれも `onerror` で自前リトライするため、再接続の試行は必ず発生する。見るのは「30 秒待たずに始まるか」と「10 回で止まるか」の 2 点。
  - **3 分経過前に「まだ続いている」ことを FAIL としない。** 上記タイムラインどおりに試行間隔が伸びていれば正常である。逆に **181 秒を過ぎても 11 回目以降が発生する場合は FAIL**（上限が効いていない）。
  - 途中で Preserve log を切ったりページを再読み込みしたりすると試行回数が数え直しになるので触らない。
